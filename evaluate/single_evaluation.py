...
def has_non_empty_values(dictionary):
    for key, value in dictionary.items():
        if value:  # 这里使用了Python的“truthiness”检查，它会判断值是否为真（非空）
            return True
    return False
...
def match_instances(real, pred, is_other=False):
    ...
    if not real or not pred:
        return [], 0, len(pred), len(real)  # 如果真实或预测为空，返回空匹配
...
# ==================== 结果输出 ====================
print(f"[类别标注分类指标]")
print(f"各部件F1: {[f'{s:.2%}' for s in part1_scores]}")
print(f"平均F1: {part1_score:.2%}")

print(f"\n[类别的坐标和标注质量]")
print(f"有效匹配数量: {len(part2_scores)}")
print(f"平均质量分: {part2_score:.2%}")

print(f"\n[特殊标注F1]")
print(f"空间标注F1: {f1[1]:.2%}")
print(f"其他标注F1: {f1[0]:.2%}")

print(f"\n[最终得分]")
print(f"加权总分: {total_score:.2%}")
